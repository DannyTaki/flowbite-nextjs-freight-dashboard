...
export const product_freight_links = pgTable(
  "product_freight_links",
  {
    ...
    classification_id: integer("classification_id").references(
      () => freight_classifications.classification_id,
    ),
    objectID: uuid("objectID").defaultRandom().notNull(),
  },
  (table) => {
    return {
      unique_product_id: unique("unique_product_id").on(table.product_id),
    };
  },
);
